...
- `responsive.css`: desktop chapters fill the viewport below the sticky header. A shared `--screen-unit` grows type, cards, controls, and scenes using both viewport width and height, with compact rules for short windows. Mobile keeps the stacked layout.
